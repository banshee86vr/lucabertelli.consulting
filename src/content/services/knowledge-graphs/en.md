...
tagline: "Small, deterministic graphs of how your systems actually work, so people and agents query facts instead of guessing."
...
faq:
  ...
  - question: "When is a light knowledge graph worth building?"
    answer: "When the answers you need live in thousands of files, several tools, and a few people's heads, and when guessing is expensive. Typical signals: a platform migration nobody can size, agents that invent inventory, or the same question answered differently depending on who you ask. Below a handful of systems that one person still holds in their head, a spreadsheet is cheaper."
  ...
  - question: "Why do large CI/CD platform migrations stall without one?"
    answer: "Because the work is not writing the next pipeline. It is knowing what already exists, what the target platform already covers, what is missing, and which work units look alike. Without that, teams either freeze or rewrite everything from scratch. I have seen migrations that looked impossible on a file count become a sequenced plan once coverage and gaps were queryable. That is the case where a light knowledge graph stops being optional."
  - question: "What do we keep after the engagement?"
    answer: "The schema, the extractors, the mapping, the query surface and the refresh path. The internal team should be able to rebuild the graph from sources without me. If the graph only exists while I am on the call, the engagement failed."
---

## The problem I usually find

The map of how a platform actually works lives in the wrong places: a few people's heads, a wiki that drifted, and thousands of files nobody wants to grep again. When a team asks "what do we run, what is already covered, what would block a move", the honest answer is a week of archaeology and three conflicting opinions.

Agents make the gap louder. Given a repository they will invent an inventory that reads well and is wrong in the places that matter. More context in the prompt does not fix that. They need facts they can query, with types and relationships that do not change meaning between calls.

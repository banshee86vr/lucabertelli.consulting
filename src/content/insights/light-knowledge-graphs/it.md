...
LA migrazione di una piattaforma CI/CD legacy quasi sempre parte in salita, se non fallisce, quando nessuno sa rispondere a quattro domande senza una settimana di archeologia:
...
È il punto in cui una [consulenza sui knowledge graph](/it/servizi/consulenza-knowledge-graph/) smette di essere un esercizio di ricerca e diventa la condizione che rende la migrazione sostenibile.
...
Non è un programma enterprise di knowledge graph. Non è Neo4j come scelta tecnologica. Non è RDF (Resource Description Framework) dove si rappresentano i dati in forma di triplette soggetto-predicato-oggetto, tipicamente usato nel contesto del Semantic Web e delle ontologie formali. Qui parliamo di un approccio molto più pragmatico e leggero, non di un framework semantico. Non è nemmeno un secondo wiki che si chiede alle persone di tenere allineato a mano. Le definizioni dei nodi, delle loro proprietà e delle loro relazioni vengono definiti ad hoc calando le scelte in base al contesto specifico del cliente e degli use-case.
...
- il refresh è gestibile con pochissime risorse e non necessità di grosse moli di risorse come per il refresh di RAG/graphRAG.
...
Un mapping per capability è quella join. I primitivi sul lato sorgente (uno step, un task, un CLI) e i provider sul lato destinazione (un riusabile, un template) si incontrano sullo stesso intento. Allora la copertura diventa è una semplice query. Le lacune si ordinano per quanto lavoro bloccano. Le pipeline simili diventano modelli riconducibili per capability e use-case coperti.
...
La [consulenza DevOps](/it/servizi/consulenza-devops/) aiuta ad ottenere una buona pratica di delivery: pipeline che funzionano, ownership che non è una sola persona. Il [Platform Engineering](/it/servizi/consulenza-platform-engineering/) può arrivare solo quando è possibile definire dei golden path una volta che si conoscono quali percorsi vale la pena estrarre. La [governance degli agenti AI](/it/servizi/consulenza-agenti-ai/), una volta che l'evoluzeione verso l'utilizzo degli agenti è avvenuta o è in corso, è indispensabile per poter definire e mantenere la governance di policy, identità e conti economici.

Il grafo è lo strato sotto, quando agenti AI, mappatura dell'esistente e migrazioni servono la stessa verità operativa. MCP senza fatti è un'altra integrazione. Un programma di piattaforma senza inventario rischia di essere un portale fine a se stesso.
